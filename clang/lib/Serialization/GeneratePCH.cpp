...
PCHGenerator::PCHGenerator(
    ...
    : PP(PP), OutputFile(OutputFile), isysroot(isysroot.str()),
      ...
      ShouldCacheASTInMemory(ShouldCacheASTInMemory), IsForCMI(IsForCMI) {
...
void PCHGenerator::HandleTranslationUnit(ASTContext &Ctx) {
  ...
  if (PP.getLangOpts().isCompilingModule() || IsForCMI) {
  ...
  // the ModuleScopes; We do not need to output the CMI in that case.
  if (IsForCMI && SemaPtr->isModuleImplementation())
    return;
